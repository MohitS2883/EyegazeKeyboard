...
def letter(index,text,letter_light):
    ...
    # else:
    #     cv2.rectangle(keyboard,(x+thickness,y+thickness),(x+width-thickness,y+height-thickness),(255,24,0),thickness)
    ...
    # text_y = int((width+height_text)/2)+y
...
def midpoint(p1, p2):
    return int((p1.x + p2.x)/2), int((p1.y + p2.y)/2)#pixels cant be 0.5
...
while True:
    ...
    keyboard[:] = (26, 26, 26)
    frames += 1
    # removed in part 10new_frame = np.zeros((500, 500, 3), np.uint8)
    gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)


    # Draw a white space for loading bar part 10
    frame[rows - 50: rows, 0: cols] = (255, 255, 255)

    if select_keyboard_menu is True:
        draw_menu()
    
    # Keyboard selected part 10
